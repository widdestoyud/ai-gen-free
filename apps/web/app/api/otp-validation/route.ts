...
export async function POST(req: NextRequest) {
  ...
  if (!email || !code) {
    return Response.json(
      { error: { code: "A001", message: "Email dan kode OTP wajib diisi" } },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (result && typeof result === "object" && "error" in result && result.error) {
      return Response.json(
        { error: { code: "A002", message: "Kode OTP salah" } },
        { status: 400 },
      );
    }
    ...
    return Response.json(
      {
        ...
        error: { code: parsed.code || "A002", message: parsed.message || "Kode OTP salah" },
      },
      {
        status: 400,
