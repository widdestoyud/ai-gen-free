...
- Auth & User Security (ADR 0014):
  ...
  - Login via `POST /user/login`: input `email` + `password`. Login pertama kali atau ganti perangkat memicu OTP verifikasi ke email.
  ...
  - Konfigurasi terpusat: Rate limit di `packages/core/src/config/rate-limit.config.ts` dan respon di `packages/core/src/config/responses.config.ts`.
