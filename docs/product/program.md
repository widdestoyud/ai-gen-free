...
Pengguna web login dengan email + OTP, isi poin lewat top up (QRIS dikonfirmasi admin), lalu generate gambar (t2i dulu). Job jalan di background; refresh tidak membatalkan. Hasil privat 14 hari. Setelah generate sukses, user menunggu cooldown (default 12 jam, diatur admin) sebelum generate berikutnya. Gagal generate tidak memotong poin dan tidak memasang cooldown.
...
| **M2** | Wallet | Lihat saldo, buat invoice top up, admin menandai lunas |
| **M3** | Jobs dummy | Submit generate palsu, polling, mutex, cooldown |
| **M4** | Siray t2i | Generate gambar nyata, file di gallery |
| **M5** | Admin + retensi | Ubah cooldown, konfirmasi bayar, file hilang setelah 14 hari |
